<template>
    <div id="wrapper-controls">
        ...
        <button v-on:click = "$emit('handelHold')" class="control btn-hold"><i class="ion-ios-download-outline"></i>Hold</button>
        
        <input 
        v-bind:value = "finalScore" 
        v-on:input = "$emit('handelfinalScore',$event)"
...
export default {
    ...
    methods:{
        newgame(){
            this.$emit('handelNewGame');
        },
        rolldice(){
            this.$emit('handelRollDice');
        }
    }
}
</script>
